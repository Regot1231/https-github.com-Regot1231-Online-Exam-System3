...
const rules = {
  ...
  role: [
    { required: true, message: '请选择角色类型', trigger: 'blur' },//非空
  ],
  password: [
...
<template>
  <el-row class="login-page">
    <el-col :span="12" class="bg bgform"  >
...
.form {
    /* 假设你希望标题在表单内水平居中 */
    text-align: center;
    position: relative; /* 让 el-col 具有定位上下文 */
}
.title {
    /* 设置标题的样式 */
    font-size: 40px;
    ...
    color: white; /* 设置标题的颜色 */
    position: absolute;
    top: 20px; /* 标题距离父元素（el-col）顶部的距离 */
    left: 20px; /* 标题距离父元素（el-col）左侧的距离 */
    z-index: 999; /* 设置标题的层级 */
}
</style>